...
void ViewManager::PrepareSceneView()
{
	...
	if (bOrthographicProjection == false)
	{
		...
		projection = glm::perspective(glm::radians(g_pCamera->Zoom), (GLfloat)WINDOW_WIDTH / (GLfloat)WINDOW_HEIGHT, 0.2f, 100.0f);
	}
	else
	{
		...
		if (glfwGetKey(m_pWindow, GLFW_KEY_P) == GLFW_PRESS && pressP == true)
			bOrthographicProjection = true;
